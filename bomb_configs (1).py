...
###########
# functions to generate targets for toggles/wires/keypad/Button, serial means keyboard by the way.
###########
def genSerial():
...
def genTogglesTarget():
    return 4   # 0100 in binary → second switch from the left

toggles_riddle_text = (
    "Riddle:\n"
    "I'm the number of bases you must touch to score.\n"
    "What number am I?\n\n"
    "Enter answer in BINARY\n"
    "Using the switches."
)
...
def genWiresTarget():
    TOTAL_WIRES = 5
    length = randint(2, TOTAL_WIRES)
    wires = list(range(TOTAL_WIRES))
    shuffle(wires)
    return wires[:length]


# generates the keypad combination from a keyword (phone keypad / T9 mapping)
def genKeypadTarget(word="YOUTH"):
    t9 = {
        **{c: "2" for c in "ABC"},
        **{c: "3" for c in "DEF"},
        **{c: "4" for c in "GHI"},
    ...
    return "".join(t9[ch] for ch in word)

riddle_text = (
    "Riddle:\n"
    "In spring I'm loud, in summer I run,\n"
    "I learn like fire beneath the sun.\n"
    "I'm not yet wise, but I'm full of truth—\n"
    "Say my name: "
)
